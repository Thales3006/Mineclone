...
ChunkMesh::ChunkMesh(std::vector<Vertex> vertices, std::vector<unsigned int> indices,
                     std::vector<Texture> textures, int x, int z)
    : Mesh(vertices, indices, textures), x(x), z(z) {}

void ChunkMesh::render(Shader &shader, int chunkx, int chunkz) {
    shader.setMat4("model", glm::translate(glm::mat4(1.0f), glm::vec3((x - chunkx) * CHUNK_WIDTH, 0,
                                                                      (z - chunkz) * CHUNK_WIDTH)));
    draw(shader);
}

std::unique_ptr<ChunkMesh> ChunkMesh::fromChunk(Chunk &chunk, std::vector<Texture> textures) {
    ...
    return std::make_unique<ChunkMesh>(drawableFaces, indices, textures, chunk.x, chunk.z);
}
