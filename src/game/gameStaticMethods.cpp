...
void Game::mouseClickCallback(GLFWwindow *window, int button, int action,
  ...
  if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
    ...
    while (game.chunkManager
               .getBlock(player.chunkx, player.chunkz,
                         player.camera.position + dir * i)
               .ID == 0) {
      if (i.x > 6)
        return;
      i += glm::vec3(1);
    ...
    if (!Entity::colision(player.camera.position + dir * i, glm::vec3(0, 0, 0),
...
void Game::keyboardCallback(GLFWwindow *window, int key, int scancode,
  ...
  if (key == GLFW_KEY_F11 && action == GLFW_PRESS) {
    if (!fullScreen) {
      glfwSetWindowMonitor(window, game.monitor, 0, 0, mode->width,
