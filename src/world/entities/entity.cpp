...
void Entity::update(ChunkManager &chunkManager, float deltaTime) {
    ...
    glm::vec3 nV =
        glm::vec3(velocity.x >= 0 ? 1 : -1, velocity.y >= 0 ? 1 : -1, velocity.z >= 0 ? 1 : -1);
    glm::vec3 nSize =
        glm::vec3(velocity.x >= 0 ? 0 : -ceil(size.x), velocity.y >= 0 ? 0 : -ceil(size.z),
        ...
        for (int y = 0; abs(y) <= abs(dV.y) + 1; y += nV.y) {
            for (int z = 0; abs(z) <= abs(dV.z) + 1; z += nV.z) {
                ...
                if (block.solid == false)
                    continue;
                blocks.push_back(blockDist{
                    block, glm::length((colisionPos + glm::vec3(0.5f)) - (position + size * 0.5f)),
                    colisionPos});
            }
        }
    }
    ...
    sort(blocks.begin(), blocks.end(),
         [](const blockDist &b1, const blockDist &b2) { return b1.dist <= b2.dist; });
...
glm::vec3 Entity::colisionContinuous(glm::vec3 staticPos, glm::vec3 staticSize, float deltaTime) {
    ...
    if (near.x > far.y || near.x > far.z || near.y > far.x || near.y > far.z || near.z > far.x ||
        near.z > far.y)
        return glm::vec3(0.0);

    float nearTime =
        near.x > near.y ? (std::isnan(near.x) ? -1 : near.x) : (std::isnan(near.y) ? -1 : near.y);
    ...
    glm::vec3 normal;
    near.x > near.y ? near.x > near.z ? normal = glm::vec3(-1, 0, 0) : normal = glm::vec3(0, 0, -1)
...
bool Entity::colision(glm::vec3 entity1Pos, glm::vec3 entity1size, glm::vec3 entity2Pos,
